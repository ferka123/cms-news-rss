...
>(
  (
    ...
    const [isLoading, setIsLoading] = React.useState(triggerSearchOnFocus);
    ...
    const CreatableItem = () => {
      ...
      if (
        inputValue &&
        !selected.find((sel) => sel.value === inputValue) &&
        !Object.entries(selectables).find((v) =>
          v[1].some((op) => op.value === inputValue)
        )
      ) {
        return Item;
      }
      ...
      if (onSearch && !creatable && Object.keys(options).length === 0) {
        return (
          <CommandItem value="-" disabled>
            {emptyIndicator || <EmptyDefault />}
          </CommandItem>
        );
      }

      return <CommandEmpty>{emptyIndicator || <EmptyDefault />}</CommandEmpty>;
