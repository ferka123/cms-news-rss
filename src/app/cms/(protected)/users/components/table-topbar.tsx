...
import { userStateOptions } from "./options-data";
import { UserListParams } from "@/lib/user/schema";

type Props = { state: UserListTableData["state"] };
...
const TableTopBar = ({ state }: Props) => {
  ...
  return (
    <div className="flex items-center justify-between pb-4 space-x-4">
          ...
          <DropdownMenuContent align="end">
            {userStateOptions.map((option) => (
              <DropdownMenuItem
                key={option.value}
                onSelect={() =>
                  applyParam<UserListParams>(router, searchParams, {
                    status: option.value,
                  })
                }
              >
      ...
      <Link href={"/cms/users/create"} scroll={false}>
        <Button>
          <span className="hidden mr-2 md:inline">New User</span>
          <CirclePlus size={16} />
        </Button>
      </Link>
    </div>
